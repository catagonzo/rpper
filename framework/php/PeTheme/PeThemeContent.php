...
class PeThemeContent {
	...
	public function the_content_more_link_filter( $link, $more_link_text ) {

		//$link = sprintf('&nbsp;<a href="%s#more-%s" class="read-more">%s</a>',get_permalink(),$GLOBALS["post"]->ID,__("more",'Pixelentity Theme/Plugin'));

		$link = sprintf( '&nbsp;<a href="%s" class="read-more">%s</a>', get_permalink(), $more_link_text === translate( '(more&hellip;)', 'default' ) ? __("more",'Pixelentity Theme/Plugin') : $more_link_text );
